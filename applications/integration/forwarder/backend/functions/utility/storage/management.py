...
def get_user_bucket_name(
    ...
    bucket_prefix = bucket_parameters['bucket-prefix']
    ice_id = bucket_parameters['ice-id']
    user = set_formatted_user(user = bucket_parameters['user'])
    target_name = target + '-' + ice_id + '-' + user
    bucket_name = bucket_prefix + '-' + target_name
    print('Used bucket name: ' + str(bucket_name))
    return bucket_name
