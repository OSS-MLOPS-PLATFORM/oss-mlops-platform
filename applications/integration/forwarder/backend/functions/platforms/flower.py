...
def format_flower_tasks(
    ...
    # This doesn't garantee a time stamp order
    # and it doesn't specify the children names
...
def parse_flower_times(
    task_info: any
) -> any:
    # Doesn't parse other states than SUCCESS
    received = task_info['received']
    started = task_info['started']
    succeeded = task_info['succeeded']
    runtime = task_info['runtime']
    total_wait_seconds = round(started-received,5)
    total_run_seconds = round(runtime,5)
    total_task_seconds = round(succeeded-received,5)

    times = {
        'total-wait-seconds': total_wait_seconds,
        'total-run-seconds': total_run_seconds,
        'total-task-seconds': total_task_seconds
    }  
